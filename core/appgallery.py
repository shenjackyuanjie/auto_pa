...
argument.add_argument('--skip-app-categories', action='store_true')
...
async def click_bottom_bar_game():
    global main_layout_res
    if main_layout_res is None:
        await wait_back_to_main_page()

    paths = utils.find_json_value_as_path(main_layout_res, "游戏")
    value = utils.find_json_value_by_path(main_layout_res, paths[0][:-1])
    bounds = utils.parse_bounds(value['bounds'])
    await hdc.click_by_bounds(bounds)
...
async def click_game_categories():
    await wait_for_app_page()
    if await is_app_categories_page():
        return
    paths = utils.find_json_value_as_path(app_categories_res, "分类")
...
async def main(args):
    ...
    args_skip_app_categories = args.skip_app_categories
    if args_skip_app_categories:
        logger.info("跳过应用分类")
    ...
    async with anyio.create_task_group() as main_tg:
        ...
        if not args_skip_app_categories:
            await click_bottom_bar_app()
            await click_app_categories()
            await pull_app_in_categories()

        await click_bottom_bar_game()
        await click_game_categories()
        await pull_app_in_categories()
